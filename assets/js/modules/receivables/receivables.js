(function(){
  ...
  const money=v=>`à§³ ${Number(v||0).toLocaleString('en-BD',{minimumFractionDigits:2,maximumFractionDigits:2})}`;
  ...
  function ensureUI(){
    ...
    section.innerHTML=`
      <div class="ar-kpis">
        <div class="stat-card sc-gold"><div class="stat-label">Total Receivable</div><div id="arKpiOutstanding" class="stat-value">à§³ 0.00</div><div class="stat-note">Posted outstanding</div></div>
        <div class="stat-card sc-info"><div class="stat-label">Overdue</div><div id="arKpiOverdue" class="stat-value">à§³ 0.00</div><div class="stat-note">Past due date</div></div>
        <div class="stat-card sc-em"><div class="stat-label">Unallocated Advance</div><div id="arKpiAdvance" class="stat-value">à§³ 0.00</div><div class="stat-note">Customer advance</div></div>
      ...
      <div class="ar-grid">
        ...
        <div class="card"><div class="card-header"><div><div class="card-title">Invoice / Demand Entry</div><div class="card-sub">Maker-checker workflow</div></div><span class="badge bg-purple">Phase 4B</span></div><div class="card-body">
          ...
          <div class="ar-total"><span>Total</span><strong id="arTotal">à§³ 0.00</strong></div><div style="display:flex;justify-content:flex-end;margin-top:14px"><button class="btn btn-gold" onclick="ReceivablesV1.saveInvoice()">Save Draft Invoice</button></div>
        </div></div>
      </div>
      ...
      <div class="ar-grid">
        <div class="card"><div class="card-header"><div><div class="card-title">AR Aging</div><div class="card-sub">Current and overdue buckets</div></div></div><div class="card-body np"><div class="table-wrap"><table><thead><tr><th>Customer</th><th>Current</th><th>1â€“30</th><th>31â€“60</th><th>61â€“90</th><th>90+</th><th>Total</th></tr></thead><tbody id="arAgingBody"></tbody></table></div></div></div>
  ...
  function ensureNav(){
    ...
    const nav=document.createElement('div');nav.className='nav-item';nav.dataset.module='receivables';nav.innerHTML='<span class="nav-icon">â—Ž</span><span data-bn="à¦ªà§à¦°à¦¾à¦ªà§à¦¯ à¦¹à¦¿à¦¸à¦¾à¦¬" data-en="Receivables">Receivables</span>';
  ...
  function render(){
    el('arCustomerBody').innerHTML=state.customers.length?state.customers.map(c=>`<tr><td><strong>${safe(c.customer_code)}</strong></td><td>${safe(c.name)}</td><td>${safe(c.customer_type)}</td><td>${safe(c.phone||'â€”')}</td><td><span class="badge ${c.status==='active'?'bg-green':'bg-danger'}">${safe(c.status)}</span></td></tr>`).join(''):'<tr><td colspan="5" style="text-align:center;padding:24px">No customers found.</td></tr>';
    el('arInvoiceCustomer').innerHTML='<option value="">Select customer</option>'+state.customers.filter(c=>c.status==='active').map(c=>`<option value="${c.id}">${safe(c.customer_code)} â€” ${safe(c.name)}</option>`).join('');
  ...
  function printReceipt(receiptNo){window.open(`${window.location.origin}/money-receipt.html?receipt_no=${encodeURIComponent(receiptNo)}&lang=${window.S?.lang||'en'}&autoprint=1`,'_blank','noopener');}
  document.addEventListener('DOMContentLoaded',()=>{ensureUI();ensureNav();if(window.TT){TT.en.receivables='Receivables';TT.bn.receivables='à¦ªà§à¦°à¦¾à¦ªà§à¦¯ à¦¹à¦¿à¦¸à¦¾à¦¬';}});
